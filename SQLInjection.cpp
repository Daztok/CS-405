...
{
    /* DONE: Fix this method to fail and display an error if there is a suspected SQL Injection
    NOTE: You cannot just flag 1=1 as an error, since 2=2 will work just as well. You need
    something more generic */

    // Basic input validation for SQL Injection patterns
    // Looking for simple patterns like "OR", "AND", or "=" in suspicious contexts
    std::regex injection_pattern(R"(\b(or|and)\b\s+[^\s]+=[^\s]+)", std::regex::icase); //icase is case insensitive

    // Check if SQL query matches the injection pattern
    if (std::regex_search(sql, injection_pattern)) {
        std::cout << "Security Warning: Possible SQL Injection attempt detected. Query blocked.\n";
        std::cout << "Attempted Query: " << sql << std::endl;
        return false; // Abort query execution if injection pattern is found
    }
...
int main()
{
    ...
    {
        /* Input validation would go here in comination with modifying the "run_queries" function
        Since run-queries is hardcoded, there is no dynamic input to validate.
        In a real world application, the user inputs would/could be validated at this point to prevent the SQL injections with input validation. */
        run_queries(db);
    }
